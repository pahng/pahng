...
export class TeamsDao extends BaseDao {
    ...
    public get post() {
        return async (request: Request, response: Response, next: NextFunction) => {
            const { display_name, team_slogan, member_one, member_two } = request.body;
            const query = `INSERT INTO teams (display_name, team_slogan, member_one, member_two, elo_score, creation_time, update_time)
            VALUES ('${display_name}', '${team_slogan}', ${member_one}, ${member_two}, 1500, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`;

            if (display_name.length < 1 || team_slogan.length < 1 || member_one.length < 1 || member_two.length < 1) {
                response.send({ error: 'input field lengths must all be greater than 1 (for now)'});
            }

            return this.pool.query(query)
                .then(() => response.send({ result: 'success', message: `Team with display name ${display_name} was created` }))
